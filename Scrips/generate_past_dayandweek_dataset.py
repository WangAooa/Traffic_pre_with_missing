...
def main(args):
    ...
    # generate_dataset(train,  12, 12, args, 'train_preweek_data', scaler)
...
def generate_dataset(data, his_len  , pre_len, args, type, scaler):
    ...
    for i in range(start_index, batch_size):
        ...
        x_lastday_t = data[i - 288: i - 288 + his_len].reshape(his_len, n_route, 1)
        x_lastweek_t = data[i - 7 * 288: i - 7 * 288 + his_len].reshape(his_len, n_route, 1)
        x_lastday.append(x_lastday_t)
        x_lastweek.append(x_lastweek_t)
    ...
    x_lastday = np.stack(x_lastday, 0)
    x_lastweek = np.stack(x_lastweek, 0)
    ...
    np.savez(save_path, x_lastweek = x_lastweek, x_lastday = x_lastday,
             x=x, x_mask=x_mask, y_missing=y_missing, y=y, time_lag=time_lag_mx,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--traffic_df_filename",
        type=str,
        default="../nav-beijing/dataset/bj_V.csv",
