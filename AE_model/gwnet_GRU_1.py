...
def get_rnn_dimsion(self, num_layer):
    dimension = []
    first_part_num = num_layer // 2
    have_middel = num_layer % 2
    temp_dim = self.rnn_unit
    for i in range(first_part_num):
        dimension.append((temp_dim, temp_dim * 2))
        temp_dim = temp_dim * 2
    if have_middel:
        dimension.append((temp_dim, temp_dim))
    for i in range(first_part_num):
        dimension.append((temp_dim, temp_dim // 2))
        temp_dim = temp_dim // 2
    return dimension
...
class EncoderModel(nn.Module):
    def __init__(self, intput_dim, adj, rnn_unit, num_node, num_rnn_layers):
        ...
        #                             )
        dimension = get_rnn_dimsion(self.rnn_unit)

        for i in range(num_rnn_layers):
            self.gwgru.append(gwnet_LSTM_cell(dimension[i][0], dimension[i][1], self.adj, self.num_node))
            self.gwgru_reverse.append(gwnet_LSTM_cell(dimension[i][0], dimension[i][1], self.adj, self.num_node))
