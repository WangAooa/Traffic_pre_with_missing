...
class gwnet(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(self.blocks * self.layers):
            ...
            #residual = cdilation_fun(x, dilation, init_dilation, i)
            residual = x
            # dilated convolution
            ...
            # print('layer i')
            # print('after dilated conv:{}'.format(x.shape))
            ...
            #print('after gcn:{}'.format(x.shape))
            x = x + residual[:, :, :, -x.size(3):]
            #print('after resi:{}'.format(x.shape))
...
if __name__ == '__main__':
    ...
    model = gwnet(device, num_nodes=228, supports=adj, in_dim=1).to(device)
    print(model)
    x = torch.zeros((32,1,228,12)).to(device).float()
    x_mask = torch.zeros((32,1,228,12)).to(device).float()
