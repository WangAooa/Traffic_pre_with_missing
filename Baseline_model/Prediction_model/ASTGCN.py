...
class Model(nn.Module):
    ...
    def __init__(self,DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, adj_mx, num_for_predict, len_input, num_of_vertices):
        '''
        :param nb_block:
        :param in_channels:
        :param K:
        ...
        self.pastweek_model = make_model(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, adj_mx, num_for_predict, len_input, num_of_vertices)
        self.pastday_model = make_model(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
                                         adj_mx, num_for_predict, len_input, num_of_vertices)
        self.model = make_model(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
                                         adj_mx, num_for_predict, len_input, num_of_vertices)
        # self.dense_layer = nn.Linear(3, 1)
    ...
    def forward(self, x):
        ...
        :return: (B, N_nodes, T_out)
        '''
        timestep = x.shape[3] // 3
        # pastweek_x, pastday_x, x = torch.split(x,timestep, -1)
        # pastweek_output = self.pastweek_model(pastweek_x)
        # pastday_output = self.pastday_model(pastday_x)
        output = self.model(x)
        # output = torch.cat((pastweek_output,pastday_output,output),3)
